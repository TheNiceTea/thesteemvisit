...
for item in json["_items"]:
	...
	cur = conn.cursor()
	print cur.execute("""UPDATE users SET steem = u.steem + %s FROM transactions t INNER JOIN users u ON t.id = u.transactionid WHERE t.name =%s""",(amount,memo,))
	print cur.execute("""DELETE FROM transactions WHERE name LIKE %s""",(memo,))
	cur.close()
	conn.commit()
	conn.close()
